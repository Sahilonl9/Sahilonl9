## Hi there , I'm Sahil 👋

**About Me** 

📊 I'm a Data Analyst who lives to turn messy, raw data into insights companies can actually act on. Whether it's cleaning up a sales dataset or building a dashboard, my goal is always the same — improve data quality and make it tell a story.

🧠 I bring a hybrid skill set to the table: strong fundamentals in data analysis & visualization, hands-on experience with backend development, and formal training as both a Data Analyst (Paul Tech) and a Data Annotator (Anudip Foundation).

💼 I've earned two paid internships during college — one as a Data Analyst Intern and one as an SDE Intern — giving me exposure to both the analytics and engineering sides of building products.

🚀 My focus areas: summarizing data, extracting insights, and building full-stack tools (Django + React) that make data more accessible and useful for the people who need it.

🌱 Currently sharpening my backend development skills while continuing to work on real-world data projects across sales, finance, and CRM domains.

🛠️ Skills
Data & Analytics: SQL | Data Cleaning | Data Visualization | Statistical Analysis | Data Annotation 

Development: Python | JavaScript | Django | React | Backend Development Tools: Jupyter Notebook | Git/GitHub

🎓 Training & Certifications

📈 Data Analyst Training — Paul Tech
🏷️ Data Annotation Training — Anudip Foundation
💻 SDE Internship — College-sponsored paid internship
📊 Data Analyst Internship — College-sponsored paid internship

📫 Reach Me At

📧 Email: sahilonl99@gmail.com
💼 LinkedIn: https://www.linkedin.com/in/sahil-ray-124b60259/
📷 portfolio: https://sahil-ray-portfoli0.netlify.app/

⭐️ Feel free to explore my repositories, and don't hesitate to reach out if you'd like to collaborate on a data or dev project!
